# 💰 Calculadora de Salário Líquido & 13º (CLT)

Uma aplicação web interativa desenvolvida em **Python** para simular descontos trabalhistas e prever ganhos reais, incluindo o cálculo detalhado das parcelas do Décimo Terceiro Salário.

🔗 **[Acesse o App Online Aqui](https://calculadora-salario-app-bmamexgmpyq2pcrsmbximn.streamlit.app/)**
*(Substitua o link acima pelo seu link real do Streamlit após o deploy)*

## 🎯 Objetivo
Facilitar o entendimento dos descontos em folha de pagamento (CLT) através de uma interface simples e direta, aplicando as regras de tributação progressiva vigentes (2024/2025).

## 🚀 Funcionalidades
* **Cálculo de INSS Progressivo:** Aplica as alíquotas corretas faixa a faixa.
* **Cálculo de IRRF:** Identifica a base de cálculo e aplica a dedução padrão.
* **Simulador de 13º Salário:**
    * **1ª Parcela:** 50% do bruto (sem descontos).
    * **2ª Parcela:** Cálculo do imposto sobre o valor total e desconto do adiantamento.
* **Gráficos Dinâmicos:** Visualização da distribuição do salário (Líquido vs Impostos).

## 🛠️ Tecnologias Utilizadas
* **Python 3.10+**
* **Streamlit:** Framework para criação de web apps de dados.
* **Pandas:** Manipulação e estruturação dos dados para os gráficos.

